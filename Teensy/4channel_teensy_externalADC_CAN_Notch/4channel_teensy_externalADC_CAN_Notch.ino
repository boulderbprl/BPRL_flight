...
const bool NOTCH_ENABLED = true;  // set false to bypass the notch filter
...
// 2nd-order notch filter, implemented as a single biquad
...
// Designs one RBJ-cookbook notch biquad section centered at f0 for
// sample rate fs, with the given notch Q (controls notch width --
// higher Q means a narrower notch).
void setBiquadNotch(Biquad &f, double f0, double fs, double Q) {
...
void setup() {
  ...
  // 2nd-order notch, one biquad section per channel.
  for (int i = 0; i < 4; i++) {
    setBiquadNotch(notch[i], FILTER_F0_HZ, FILTER_FS_HZ, FILTER_Q);
  }
}
...
void loop() {
  if (sampleTimer >= SAMPLE_INTERVAL_US) {
    ...
    // Filter each channel: 2nd-order notch, FILTER_F0_HZ / FILTER_Q / FILTER_FS_HZ above
    int16_t filtered[4];
    for (int i = 0; i < 4; i++) {
      if (NOTCH_ENABLED) {
        double y = biquadProcess(notch[i], (double)tmp[i]);
