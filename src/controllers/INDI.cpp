...
void INDI::update(const float euler[3], const float state_full[],
                  ...
                  const Unmixer &unmixer, float out_cmds[3])
{
    ...
    const float delta_torque_roll  = (accel_cmd_roll  - p_dot_meas) * INDI_GAIN;
    const float delta_torque_pitch = (accel_cmd_pitch - q_dot_meas) * INDI_GAIN;

    const float total_torque_roll  = current_torque[0] + delta_torque_roll;
    const float total_torque_pitch = current_torque[1] + delta_torque_pitch;

    out_cmds[0] = unmixer.normalize_torque(total_torque_roll);
    out_cmds[1] = unmixer.normalize_torque(total_torque_pitch);

    // ── Yaw: standard rate PID (unchanged from SLCPID) ───────────────────
    out_cmds[2] = _yaw_rate.update(YAW_GAIN * input[3] - r);
}

float INDI::compute_throttle(const float euler[3], const float input[]) const
{
    ...
    const float boost   = 1.0f / std::min(cosf(euler[0]), cosf(euler[1]));
